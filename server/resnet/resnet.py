...
class ResNet(object):
    def __init__(self, hp, images, labels, global_step, name=None, reuse_weights=False, training=True):
        ...
        for i in range(self._hp.num_gpus):
            with tf.device('/GPU:%d' % i), tf.variable_scope(tf.get_variable_scope()):
                with tf.name_scope('tower_%d' % i) as scope:
                    ...
                    logits, preds, loss, acc = self.build_tower(self._images[i], self._labels[i] if self.training else None)
...
def _activations_summaries(activations):
    """ Creates two summaries: first provides a histogram of activations
        and second one measures theirs sparsity.
