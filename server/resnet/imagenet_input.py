...
def read_input_file(txt_fpath, dataset_root, shuffle=True):
  ...
  seed = randint(-99999999999, 99999999999)
  filename_queue = tf.train.string_input_producer([txt_fpath], shuffle=shuffle, seed=seed)
  ...
  result.image_path = dataset_root_t + parsed_entries[0]
  raw_jpeg = tf.read_file(result.image_path)
  result.image = tf.image.decode_jpeg(raw_jpeg, channels=1)
...
def resize_image(input_image, random_aspect=False):
  with tf.variable_scope('resize_image'):
    ...
    image = input_image
    image = tf.image.resize_images(input_image, [IMAGE_HEIGHT, IMAGE_WIDTH])    
    image = tf.reshape(image, [IMAGE_HEIGHT, IMAGE_WIDTH, 1]) 

    return image

def resize_image_and_batch(input_image, random_aspect=False):
  with tf.variable_scope('resize_image_and_batch'):
    # Resize image to [IMAGE_HEIGHT, IMAGE_WIDTH]
    image = input_image
    image = tf.image.resize_images(input_image, [IMAGE_HEIGHT, IMAGE_WIDTH])    
    batch = tf.reshape(image, [1, 1, IMAGE_HEIGHT, IMAGE_WIDTH, 1]) 

    return batch


def random_crop_with_resize(input_image):
  with tf.variable_scope('random_crop'):
    ...
    height_aspect_ratio = tf.random_uniform([], minval=0.8, maxval=1.0, dtype=tf.float64)
    width_aspect_ratio = tf.random_uniform([], minval=0.8, maxval=1.0, dtype=tf.float64)

    height_crop =  height_orig * height_aspect_ratio
    width_crop = width_orig * width_aspect_ratio
...
def preprocess_image(input_image):
  ...
  image = tf.image.per_image_standardization(input_image)
...
def distorted_inputs(dataset_root, txt_fpath, batch_size, shuffle=True, num_threads=60, num_sets=1):
  ...
  for i in range(num_sets):
    ...
    # Random crop
    # image = random_crop_with_resize(image)
...
def inputs(dataset_root, txt_fpath, batch_size, shuffle=False, num_threads=60, num_sets=1, center_crop=False):
  ...
  for file in [dataset_root, txt_fpath]:
